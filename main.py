...
for t in targets:
    ...
    if should_inject and not check_only:
        ...
        change_password_success = boxman.change_box_password(t, mode="USER")
        print(f"[+] User password is changed. Status: {change_password_success}")
        ...
        change_password_success = boxman.change_box_password(t, mode="ROOT")
        print(f"[+] User password is changed. Status: {change_password_success}")
        if (inject_success and change_password_success):
